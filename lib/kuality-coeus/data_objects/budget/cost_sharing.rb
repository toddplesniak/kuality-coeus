class CostSharingObject < DataFactory
  ...
  def edit(opts)
    ...
    on CostSharing do |page|
      page.period(@source_account, @amount.commas).pick! opts[:period]
      page.percentage(@source_account, @amount.commas).fit opts[:percentage]
      page.amount(@source_account, @amount.commas).fit opts[:amount]
      # NOTE: This needs to be last so that the @source_account still matches for the
      # other three fields...
      page.source_account(@source_account, @amount.commas).fit opts[:source_account]
      page.save
    end
    update_options(opts)
  end
