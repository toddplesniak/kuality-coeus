...
Feature: Proposal Actions Validations
  ...
  Scenario: A PI has not been added to the proposal
    ...
    Then  the Proposal's data validation shows an error that says there is no principal investigator

  Scenario: Sponsor deadline date is missing
    Given the Proposal Creator creates a Proposal without a sponsor deadline date
    When  data validation is activated
    Then  the Proposal's data validation shows an error that says sponsor deadline date not entered
  ...
  Scenario: Sponsor deadline date has passed
    ...
    When  data validation is activated
    Then  the Proposal's data validation shows an error that says the sponsor deadline has passed
